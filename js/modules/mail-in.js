...
window.modules['mail-in'] = {
            ...
            <div class="table-container">
                <div class="table-actions">
                    ...
                    <button id="add-mail-btn" class="btn btn-primary">
                        <i class="ph ph-plus"></i> Catat Surat Masuk
                    </button>
                </div>
    ...
    renderTable: async function () {
        ...
        try {
            let items = await window.db.getAll('mail_in');
            ...
            if (filtered.length === 0) {
                tbody.innerHTML = '<tr><td colspan="6" class="text-center text-muted">Tidak ada surat masuk.</td></tr>';
                return;
            }
    ...
    openForm: async function (id = null) {
        ...
        if (id) {
            ...
            item = await window.db.get('mail_in', id);
        } else {
            modalTitle.textContent = 'Catat Surat Masuk';
        }
        ...
        modalBody.innerHTML = `
            <form id="mail-form">
                ...
                <div class="form-group">
                    ...
                    <input type="text" id="sender" class="form-control" value="${item.sender}" required>
                </div>
                <div class="form-group">
                    <label>Perihal</label>
                ...
                <div class="form-group">
                    ...
                    <textarea id="description" class="form-control" rows="3">${item.description}</textarea>
                </div>
                <div class="form-group">
        ...
        newSaveBtn.addEventListener('click', async () => {
            ...
            try {
                if (id) {
                    await window.db.update('mail_in', { ...formData, id });
                    app.showToast('Surat masuk berhasil diperbarui');
                } else {
                    await window.db.add('mail_in', formData);
                    app.showToast('Surat masuk berhasil dicatat');
                }
    ...
    deleteItem: async function (id) {
        if (confirm('Hapus surat ini?')) {
            try {
                await window.db.delete('mail_in', id);
